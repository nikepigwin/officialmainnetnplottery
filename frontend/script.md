...
function getSelectedTokenInfo() {
  // Always use '' (empty string) for ADA policyId
  return acceptedTokens.find(t => t.policyId === selectedToken) || 
         { policyId: '', symbol: 'ADA', name: 'ADA', price: 1, decimals: 6 };
}
...
async function initializeLucid() {
  ...
  console.log('Current window.Lucid:', window.Lucid);
  console.log('Current window.Blockfrost:', window.Blockfrost);
  
  try {
    // Wait for Lucid to be available
    let attempts = 0;
    while (attempts < 50 && !window.Lucid) {
      console.log(`Attempt ${attempts + 1}: Waiting for Lucid...`);
      await new Promise(resolve => setTimeout(resolve, 100));
      attempts++;
    }
    
    if (!window.Lucid) {
      console.error('❌ Lucid not found after 50 attempts');
      console.log('Available window properties:', Object.keys(window).filter(key => key.toLowerCase().includes('lucid')));
      throw new Error('Lucid not available - check if script loaded correctly');
    }
    
    console.log('✅ Found window.Lucid');
    ...
    if (!configResponse.ok) {
      throw new Error(`Failed to fetch Blockfrost config: ${configResponse.status}`);
    }
    ...
    if (!config.projectId) {
      throw new Error('Blockfrost API key not configured. Please set BLOCKFROST_API_KEY in backend environment.');
    }
    
    // Initialize Lucid with web module API structure
...
async function buyTicketsForLottery(ticketCount) {
  try {
    ...
    // (You may need to update the datum update logic here to match your backend)
    // ...
  } catch (error) {
...
function setupEventListeners() {
  ...
  if (buyTicketsBtn) {
    ...
    buyTicketsBtn.addEventListener('click', async (e) => {
      ...
      // Store the wallet address at the time of click
      const purchaseWalletAddress = window.currentUserAddress;
      try {
        buyTicketsBtn.textContent = 'Processing...';
        // buyTicketsBtn.disabled = true; // keep enabled for UX
        
        // Set up a watcher to revert the button if wallet changes
        let walletChanged = false;
        const walletCheckInterval = setInterval(() => {
          if (window.currentUserAddress !== purchaseWalletAddress || !connectedWallet) {
            buyTicketsBtn.textContent = 'Buy Tickets';
            buyTicketsBtn.disabled = false;
            walletChanged = true;
        ...
        const prevPool = await getCurrentPoolAmount();
        const txHash = await buyTicketsForLottery(ticketCount);
        
        // Reset form
        ticketCountInput.value = 1;
        updateTotalCost();
        
        // After purchase, check if pool increased and show public notification
        const newPool = await getCurrentPoolAmount();
        if (newPool > prevPool) {
          showNotification(`Pool increased by ${(newPool - prevPool).toFixed(2)} ADA!`, 'info');
        }
...
async function initializeSmartContract() {
  ...
  try {
    ...
    const SCRIPT_ADDRESS = scriptAddress; // Use the same script address as buyTicketsForLottery
    const MIN_ADA = 2000000n;
    // Minimal datum only
    const datum = {
      total_pools: [["", 0n]],
      ticket_prices: [["", 1000000n]],
      total_tickets: 0n,
      accepted_tokens: [""],
      prize_split: [["", [100n]]]
    };
    const datumType = window.Lucid.Data.Object({
      total_pools: window.Lucid.Data.Array(window.Lucid.Data.Tuple([window.Lucid.Data.Bytes(), window.Lucid.Data.Integer()])),
      ticket_prices: window.Lucid.Data.Array(window.Lucid.Data.Tuple([window.Lucid.Data.Bytes(), window.Lucid.Data.Integer()])),
      total_tickets: window.Lucid.Data.Integer(),
      accepted_tokens: window.Lucid.Data.Array(window.Lucid.Data.Bytes()),
      prize_split: window.Lucid.Data.Array(window.Lucid.Data.Tuple([window.Lucid.Data.Bytes(), window.Lucid.Data.Array(window.Lucid.Data.Integer())]))
    });
    const address = await lucid.wallet.address();
    const utxos = await lucid.wallet.getUtxos();
    ...
    const tx = await lucid
      ...
      .payToContract(
        SCRIPT_ADDRESS,
        { inline: window.Lucid.Data.to(datum, datumType) },
        { lovelace: MIN_ADA }
      )
      .complete();
...
function cleanupPriceUpdates() {
    if (priceUpdateInterval) {
        ...
        console.log('🧹 Price update interval cleaned up');
    }
}
